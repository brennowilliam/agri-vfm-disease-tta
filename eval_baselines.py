...
def linear_probe(backbone: str, target: str = "plantdoc"):
    Xtr, ytr = _load(backbone, "plantvillage")
    Xte, yte = _load(backbone, target)
    Xtr, Xte = normalize(Xtr), normalize(Xte)
    clf = LogisticRegression(max_iter=2000, C=1.0, n_jobs=-1)
    clf.fit(Xtr, ytr)
    M.print_summary(f"{backbone} linear-probe (PV->{target}, field)",
                    M.summarize(yte, clf.predict(Xte), NUM_CLASSES, IDX_TO_CLASS))


def knn(backbone: str, target: str = "plantdoc", k: int = 20):
    Xtr, ytr = _load(backbone, "plantvillage")
    Xte, yte = _load(backbone, target)
    Xtr, Xte = normalize(Xtr), normalize(Xte)
    clf = KNeighborsClassifier(n_neighbors=k, metric="cosine", n_jobs=-1)
    clf.fit(Xtr, ytr)
    M.print_summary(f"{backbone} kNN(k={k}) (PV->{target}, field)",
                    M.summarize(yte, clf.predict(Xte), NUM_CLASSES, IDX_TO_CLASS))


def clip_zeroshot(target: str = "plantdoc"):
    ...
    Xte, yte = _load("clip_vitb32", target)
    ...
    M.print_summary(f"CLIP zero-shot ({target}, field)",
...
def main():
    ...
    if args.clip_zeroshot:
        clip_zeroshot(args.target)
    else:
        linear_probe(args.backbone, args.target)
        knn(args.backbone, args.target)
